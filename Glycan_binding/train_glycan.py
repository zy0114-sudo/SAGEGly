...
dataset_train=MyOwnDataset('/Glycan_binding/train_pt')


#test
file1='/Glycan_binding/test_npz'
...
dataset_test=MyOwnDataset2('/Glycan_binding/test_pt')
...
for epoch in range(config_runtime['num_epochs']):
   ...
   # train_recall = a3[1]
    ...
    '''
    output_path='/Glycan_binding/model'
    #if test_acc >= 0.7:
    #    print('测试集的准确率为%s，在第%d中'%(t_acc,epoch))
    torch.save(model.state_dict(), "/Glycan_binding/model/%d_model.pt"%epoch)
    ...
    print(f'Epoch: {epoch}, train_loss: {train_loss:.4f},train_Acc: {train_acc:.4f},train_AUC: {train_auc:.4f},train_prescision:{train_precision:.4f},train_recall:{train_recall:.4f},train_mcc:{train_mcc:.4f},train_f1:{train_f1:.4f}',flush=True)
    print(f'test_Acc: {test_acc:.4f},test_AUC: {test_auc:.4f},test_prescision:{test_precision:.4f} ,test_recall:{test_recall:.4f},test_mcc:{test_mcc:.4f},test_f1:{test_f1:.4f}',flush=True)
    print('===========================================',flush=True)
